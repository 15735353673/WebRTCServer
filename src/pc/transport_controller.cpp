/*************************************************************************
	> File Name: transport_controller.cpp
	> Author: lingye
	> Mail: 15735353673@163.com 
	> Created Time: Mon 14 Jul 2025 06:51:01 PM CST
 ************************************************************************/
...
namespace xrtc
{
	...
	int TransportController::set_local_description(SessionDescription* desc)
	{
		...
		for(auto content : desc->contents())
		{
			...
			if(td)
			{
				_ice_agent->set_ice_param(mid,IceCandidateComponent::RTP,IceParameters(td->ice_ufrag,td->ice_pwd));
			}
	...
	void TransportController::_on_ice_state(IceAgent*, IceTransportChannelState) 
	{
		_update_state();
	}
	...
	void TransportController::_update_state()
	{
		...
		auto it = _dtls_transport_by_name.begin();
		for(; it!= _dtls_transport_by_name.end();it++)
		{
			dtls_state_counts[it->second->dtls_state()]++;
			ice_state_counts[it->second->ice_channel()->state()]++;
		}

		int total_connected = ice_state_counts[IceTransportChannelState::k_connected] +
		...
		int total_failed = ice_state_counts[IceTransportChannelState::k_failed] + 
			dtls_state_counts[DtlsTransportState::k_failed];
		int total_closed = ice_state_counts[IceTransportChannelState::k_closed] + 
			dtls_state_counts[DtlsTransportState::k_closed];
		int total_new = ice_state_counts[IceTransportChannelState::k_new] + 
	...
	{
		auto it = _dtls_transport_by_name.find(transport_name);
		if(it != _dtls_transport_by_name.end())
		{
			return it->second;
		}
	...
	int TransportController::set_remote_description(SessionDescription* desc)
	{
		...
		for(auto content : desc->contents())
		{
			...
			if(td)
			{
				_ice_agent->set_remote_ice_param(mid,IceCandidateComponent::RTP,IceParameters(td->ice_ufrag,td->ice_pwd));
