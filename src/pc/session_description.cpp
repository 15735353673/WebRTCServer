/************************************************************************
	> File Name: session_description.cpp
	> Author: lingye
	> Mail: 15735353673@163.com 
	> Created Time: Sat 12 Jul 2025 12:06:17 PM CST
 ************************************************************************/
...
namespace xrtc
{
	...
	VideoContentDescription::VideoContentDescription()
	{
		...
		auto rtc_codec = std::make_shared<VideoCodecInfo>();
		rtc_codec->id = 99;
		rtc_codec->name = "rtx";
		rtc_codec->clockrate = 90000;

		// add codec param
		rtc_codec->codec_param["apt"] = std::to_string(codec->id);

		_codecs.push_back(rtc_codec);
	}
	...
	std::string SessionDescription::to_string()
	{
		...
		// session origin
		// RFC 4566
		// o=<username> <sess-id> <sess-version> <nettype> <addrtype> <unicastaddress>
		...
		std::vector<const ContentGroup*> content_groups = get_group_by_name("BUNDLE");
		if(!content_groups.empty())
		{
			ss << "a=group:BUNDLE";
			for(auto group : content_groups)
			{
