/*************************************************************************
	...
	> Created Time: Fri 04 Jul 2025 10:31:53 PM CST
 ************************************************************************/
...
namespace xrtc
{
	...
	int create_tcp_server(const char * addr, int port)
	{
		...
		sa.sin_addr.s_addr=htonl(INADDR_ANY); //本地字节序转网络字节序
		...
		if (-1 ==ret)
		{
			RTC_LOG(LS_WARNING)<<"bind port  error, errno: "<<errno
				<<",error:"<<strerror(errno);
			return -1;
		}
		...
		if (-1 ==ret)
		{
			RTC_LOG(LS_WARNING)<<"listen port error, errno: "<<errno
				<<",error:"<<strerror(errno);
			return -1;
		}
		return sock;
	...
	int generic_accept(int sfd,sockaddr * sa, socklen_t* len)
	{
		int fd=-1;
		while(1)
		{
			fd=accept(sfd,sa,len);
			if(-1 == fd)
			{
				if(EINTR == errno)
				{
	...
	int tcp_accept(int sfd,char * host,int * port)
	{
		struct sockaddr_in sa;
		socklen_t salen=sizeof(sa);

		int fd=generic_accept(sfd,(struct sockaddr*)&sa,&salen);
	...
	int sock_setnonblock(int fd)
	{
		...
		if(-1 == flags)
		{
			RTC_LOG(LS_WARNING) << "fctnl(F_GETFL) error:" << strerror(errno)
				<< ", errno:" << errno;
			return -1;
		}
		...
		{
			RTC_LOG(LS_WARNING) << "fctnl(F_SETFL) error:" << strerror(errno)
				<< ", errno:" << errno;
			return -1;
		}
	...
	int sock_peer_to_str(int fd,char* ip,int* port)
	{
		...
		int ret= getpeername(fd,(struct sockaddr*)&sa,&salen);
		if(-1 == ret)
		{
			if(ip)
			{
	...
	int sock_read_data(int fd, char * buf, size_t len)
	{
		...
		if(-1 == nread)
		{
			...
			{
				RTC_LOG(LS_WARNING) << "cosk read failed, error:" << strerror(errno)
					<< ", errno" << errno;
				return -1;
			}
		}
	...
	int sock_recv_from(int sock,char * buf,size_t size,struct sockaddr* addr,socklen_t addr_len)
	{
		...
		if(received < 0 )
		{
			...
			{
				RTC_LOG(LS_WARNING) << "reecv from error:" << strerror(errno)
					<<", errno:" << errno;
				return -1;
			}
		} 
