        ...
        Candidate& c) 
{
	...
	//改为本地公网ip
    _local_addr.SetIP("101.126.155.67");
...
IceConnection* UDPPort::create_connection(EventLoop* el,const Candidate& candidate)
{
	IceConnection* conn = new IceConnection(el,this,candidate);
...
bool UDPPort::get_stun_message(const char* data,size_t len,const rtc::SocketAddress& addr, std::unique_ptr<StunMessage>* out_msg,
		std::string* out_username)
{
	...
	if(STUN_BINDING_REQUEST == stun_msg->type())
	{
		...
		{
			//todo:发生错误的响应
			RTC_LOG(LS_WARNING) << to_string() << ": recrvied "
				<<stun_method_to_string(stun_msg->type())
		...
		if(!_parse_stun_username(stun_msg.get(),&local_ufrag,&remote_ufrag))
		{
			RTC_LOG(LS_WARNING) << to_string() << ": recrvied "
				<<stun_method_to_string(stun_msg->type())
				<<" with bad local ufrag: " << local_ufrag
				<<" form" <<addr.ToString();
		...
		const std::string& reason)
{
	...
	if(err_code != STUN_ERROR_BAD_REQUEST && err_code != STUN_ERROR_UNATHORIZED)
	{
