/*************************************************************************
	> File Name: ice_agent.cpp
	> Author: lingye
	> Mail: 15735353673@163.com 
	> Created Time: Mon 14 Jul 2025 12:58:34 PM CST
 ************************************************************************/
...
namespace xrtc
{
	IceAgent::IceAgent(EventLoop* el,PortAllocator* allocator):_el(el),
	...
	void IceAgent::on_candidate_allocator_done(IceTransportChannel* channel,const std::vector<Candidate>& candidates)
	{
	...
	void IceAgent::_update_state() {
		IceTransportChannelState ice_state = IceTransportChannelState::k_new;
		std::map<IceTransportChannelState, int> ice_state_counts;
		for (auto channel : _channels) {
			ice_state_counts[channel->state()]++;
		}

		int total_ice_new = ice_state_counts[IceTransportChannelState::k_new];
		int total_ice_checking = ice_state_counts[IceTransportChannelState::k_checking];
		int total_ice_connected = ice_state_counts[IceTransportChannelState::k_connected];
		int total_ice_completed = ice_state_counts[IceTransportChannelState::k_completed];
		int total_ice_failed = ice_state_counts[IceTransportChannelState::k_failed];
		int total_ice_disconnected = ice_state_counts[IceTransportChannelState::k_disconnected];
		int total_ice_closed = ice_state_counts[IceTransportChannelState::k_closed];
		int total_ice = _channels.size();

		if (total_ice_failed > 0) {
			ice_state = IceTransportChannelState::k_failed;
		} else if (total_ice_disconnected > 0) {
			ice_state = IceTransportChannelState::k_disconnected;
		} else if (total_ice_new + total_ice_closed == total_ice) {
			ice_state = IceTransportChannelState::k_new;
		} else if (total_ice_new + total_ice_checking > 0) {
			ice_state = IceTransportChannelState::k_checking;
		} else if (total_ice_completed + total_ice_closed == total_ice) {
			ice_state = IceTransportChannelState::k_completed;
		} else if (total_ice_connected + total_ice_completed + total_ice_closed == total_ice) {
			ice_state = IceTransportChannelState::k_connected;
		}

		if (_ice_state != ice_state) {
			// 为了保证不跳过k_connected状态
			if (_ice_state == IceTransportChannelState::k_checking
					&& ice_state == IceTransportChannelState::k_completed)
			{
				signal_ice_state(this, IceTransportChannelState::k_connected);
			}
		...
		IceCandidateComponent component)
	{
		...
		channel->signal_candidate_allocate_done.connect(this,&IceAgent::on_candidate_allocator_done);
	...
	void IceAgent::set_ice_param(const std::string& transport_name,
			IceCandidateComponent component,
	...
	void IceAgent::set_remote_ice_param(const std::string& transport_name,
			IceCandidateComponent component,
			const IceParameters& ice_params)
	{
		auto channel = get_channel(transport_name,component);
