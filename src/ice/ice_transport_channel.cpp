...
void IceTransportChannel::set_remote_ice_params(const IceParameters& ice_params) {
	...
	_sort_connection_and_update_state();
}
...
void IceTransportChannel::gathering_candidate() {
    ...
    auto network_list = _allocator->get_network();
...
void IceTransportChannel::_sort_connection_and_update_state()
{
...
void IceTransportChannel::_update_state()
{
	...
	IceTransportChannelState state = _compute_ice_transport_state();
	if(state != _state)
	{
		_state = state;
...
IceTransportChannelState IceTransportChannel::_compute_ice_transport_state()
{
	...
	{
		return IceTransportChannelState::k_failed;
	}

	if(_has_been_connection && !writable())
	{
		return IceTransportChannelState::k_disconnected;
	}

	if(!_had_connection && !has_connection)
	{
		return IceTransportChannelState::k_new;
	}

	if(has_connection && !writable())
	{
		return IceTransportChannelState::k_checking;
	}

	return IceTransportChannelState::k_connected;
}
...
{
	std::stringstream ss;
	ss<< " Channel[" << this << ":" << _transport_name
		<< ":" << _component << "]";
	return ss.str();
}
