/*************************************************************************
	> File Name: src/pc/dtls_transport.cpp
	> Author: lingye
	> Mail: 15735353673@163.com 
	> Created Time: Thu 31 Jul 2025 01:20:16 PM CST
 ************************************************************************/
...
namespace xrtc
{
	...
	{
		rtc::Buffer remote_fignerprint_value(digest,digest_len);

		if(_dtls_active && _remote_fingerprint_value == remote_fignerprint_value 
				&& !digest_alg.empty())
		{
			//已经设置过了
		...
		_remote_fingerprint_value = std::move(remote_fignerprint_value);
	...
	void DtlsTransport::_set_writable_state(bool state)
	{
		if(_writable == state)
		{
			return ;
		}
		 RTC_LOG(LS_INFO) << to_string() << ": set DTLS writable to " << state;
		 _writable = state;
