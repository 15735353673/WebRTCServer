/*************************************************************************
	> File Name: async_udp_socket.cpp
	> Author: lingye
	> Mail: 15735353673@163.com 
	> Created Time: Sat 19 Jul 2025 04:01:28 PM CST
 ************************************************************************/
...
namespace xrtc
{
	...
	void async_udp_socket_io_cb(EventLoop* /*el*/, IOWatcher* /*w*/, int /*fd*/, int events,void * data)
	{
		...
		else if(events & EventLoop::WRITE)
		{
			udp_socket->send_data();
		}
	}

	AsyncUdpSocket::AsyncUdpSocket(EventLoop* el, int socket):_el(el),
		_socket(socket),
