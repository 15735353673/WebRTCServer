/*************************************************************************
	> File Name: signaling_worker.cpp
	> Author: lingye
	> Mail: 15735353673@163.com 
	> Created Time: Mon 07 Jul 2025 02:15:51 PM CST
 ************************************************************************/
...
namespace xrtc
{
	...
	SignalingWorker::SignalingWorker(int worker_id, const SignalingServerOptions& options):_worker_id(worker_id),
		_options(options),
	...
	int SignalingWorker::init()
	{
		...
		if(pipe(fds))
		{
			RTC_LOG(LS_WARNING) << "create pipe failed, error:"<< strerror(errno)
				<<", errno:"<<errno;
			return -1;
		}
	...
	int SignalingWorker::start()
	{
		if(_thread)
		{
			RTC_LOG(LS_WARNING) << "signaling work alreadly start,worker_id:"<<_worker_id;
			return -1;
		}

		_thread = new std::thread([=](){
				RTC_LOG(LS_INFO) << "signaling worker loop start, worker_id:"<<_worker_id;
				_el->start();
				RTC_LOG(LS_INFO) << "signaling worker loop stop, worker_id:"<<_worker_id;
				});
		return 0;
	...
	void SignalingWorker::_response_server_offer(std::shared_ptr<RtcMsg> msg)
	{
		...
		//将offer传给tcpconnection
		TCPConnection* c= (TCPConnection*)msg->conn;
		//注意这个c有可能在这个阶段已经关闭，使用它的时候可能出现野指针
		if(!c)
		{
			return ;
		}
	...
	{
		RTC_LOG(LS_INFO) << "signaling worker worker_id:" <<_worker_id << "   receive read event, fd:"<< fd; 
		//读取网络数据
		if (fd<0 ||(size_t)fd >= _conns.size())
		{
	...
	int SignalingWorker::_process_request(TCPConnection* c,const rtc::Slice& header,const  rtc::Slice& body)
	{
		...
		if(!err.empty())
		{
			RTC_LOG(LS_WARNING) << "parse json body error:" << err << ", fd"
				<< c->fd << ", log_id" << head->log_id;
			return -1;
		}
		...
		{
			RTC_LOG(LS_WARNING) << "no cmdno field in body , log_id:" << head->log_id;
			return  -1;
		}
		...
		{
			case CMDNO_PUSH:
				return _process_push(cmdno,c,root,head->log_id);
			case CMDNO_PULL:
				return _process_pull(cmdno,c,root,head->log_id);
			case CMDNO_STOP_PUSH:
				ret = _process_stop_push(cmdno, c, root, head->log_id);
				break;
			case CMDNO_STOPPULL:
				ret = _process_stop_pull(cmdno, c, root, head->log_id);
				break;
			case CMDNO_ANSWER:
				ret = _process_answer(cmdno,c,root,head->log_id);
				break;
			default:
				ret = -1;
				RTC_LOG(LS_WARNING) << "unknown cmdno:" << cmdno <<  ",log_id" << head->log_id;
				break;
		}
	...
	int SignalingWorker::_process_push(int cmdno,TCPConnection* c,const Json::Value& root,uint32_t log_id)
	{
		...
		RTC_LOG(LS_INFO) << "cmdno["<<cmdno << "] uid[" << uid << "] stream_name["
			<<stream_name << "] video[" << video << "] audio[" << audio
			<<"] signaling server push request";

		//将数据交给rtcserver  通过管道+消息队列  首先将数据打包
	...
	int SignalingWorker::_process_stop_push(int cmdno, TCPConnection* ,
	...
	int SignalingWorker::_process_answer(int cmdno,TCPConnection* ,const Json::Value& root,uint32_t log_id)
	{
		uint64_t uid;
	    std::string stream_name;
	    std::string answer;
